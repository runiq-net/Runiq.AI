# Runiq.AI.ContextSpaces

![NuGet Version](https://img.shields.io/nuget/vpre/Runiq.AI.ContextSpaces?label=nuget)

Context spaces and source-reading primitives for Runiq AI agents.

`Runiq.AI.ContextSpaces` provides the building blocks for attaching read-only domain context to agents. Use it to define reusable context spaces, register source providers, discover skill documents, and expose supported source content through the Runiq AI runtime and dashboard.

## Why Runiq.AI.ContextSpaces?
...
`Runiq.AI.ContextSpaces` keeps that context separate from agent definitions.
...
This keeps your domain material outside the agent definition while still making it available to the Runiq AI runtime.
...
Use `Runiq.AI.ContextSpaces` when you want to:
...
Runiq AI is modular. `Runiq.AI.ContextSpaces` can be used together with other Runiq packages:

| Package | Purpose |
|---|---|
| `Runiq.AI.Agents` | Defines agents and tool execution primitives |
