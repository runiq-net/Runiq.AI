...
public sealed class RagService : IRagService
{
    ...
    public RagService(IRagRetriever retriever)
    {
        ...
        documentIngestion = null!; upsertPipeline = null!; vectorStore = null!; state = null!; ingestionOptions = new RagIngestionOptions();
    }
    ...
    public RagService(IRagRetriever retriever, IRagDocumentIngestionService documentIngestion, IRagVectorStoreUpsertPipeline upsertPipeline, IRagVectorStore vectorStore, RagIngestionState state, IOptions<RagIngestionOptions>? ingestionOptions = null)
    {
        this.retriever = retriever ?? throw new ArgumentNullException(nameof(retriever));
    ...
    public async Task<RagIngestionReport> IngestAsync(IRagDocumentSource source, string indexName, CancellationToken cancellationToken = default)
    {
        ...
        {
            try { var outcome = await IngestCoreAsync(document, indexName, sourceKey, token).ConfigureAwait(false); lock (failures) { created += outcome.Created; updated += outcome.Updated; skipped += outcome.Skipped; chunks += outcome.Chunks; } }
        ...
        var deleted = ingestionOptions.PropagateDeletes ? await PropagateDeletesAsync(sourceKey, indexName, documents.Select(x => x.Id), cancellationToken).ConfigureAwait(false) : 0;
    ...
    private async Task<(int Created, int Updated, int Skipped, int Chunks)> IngestCoreAsync(RagSourceDocument source, string indexName, string sourceKey, CancellationToken token)
    {
        ...
        var result = await documentIngestion.IngestAsync(document, token).ConfigureAwait(false);
        if (result.Items.Count == 0) return (0, 0, 1, 0);
        var dimensions = result.Items[0].EmbeddingResult.Embedding.Dimensions;
        var index = await vectorStore.CreateIndexAsync(new CreateVectorIndexRequest { IndexName = indexName, Dimensions = dimensions }, token).ConfigureAwait(false);
        if (!index.Succeeded) throw new InvalidOperationException(index.Reason);
        var persisted = await upsertPipeline.UpsertAsync(result, indexName, document.Metadata, dimensions, token).ConfigureAwait(false);
        ...
        if (old is not null && old.ChunkIds.Count > 0) await vectorStore.DeleteAsync(new DeleteVectorRequest { IndexName = indexName, VectorIds = old.ChunkIds.ToList() }, token).ConfigureAwait(false);
    ...
    private async Task<int> PropagateDeletesAsync(string sourceKey, string indexName, IEnumerable<string> present, CancellationToken token) { var ids = new HashSet<string>(present, StringComparer.Ordinal); List<(string Key, RagIngestionState.Entry Entry)> stale; lock (state.Gate) stale = state.Entries.Where(x => x.Key.StartsWith(indexName + "|" + sourceKey + "|", StringComparison.Ordinal) && !ids.Contains(x.Key[(indexName.Length + sourceKey.Length + 2)..])).Select(x => (x.Key, x.Value)).ToList(); foreach (var item in stale) { var result = await vectorStore.DeleteAsync(new DeleteVectorRequest { IndexName = indexName, VectorIds = item.Entry.ChunkIds.ToList() }, token).ConfigureAwait(false); if (!result.Succeeded) throw new InvalidOperationException(result.Reason); lock (state.Gate) state.Entries.Remove(item.Key); } return stale.Count; }
