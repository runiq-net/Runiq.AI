...
Embedded ASP.NET Core runtime and dashboard hosting layer for Runiq.Net.

`Runiq.Core` wires Runiq.Net into ASP.NET Core applications. Use it to register agents, expose runtime endpoints, host the embedded dashboard, browse context spaces, and inspect workflow activity from your own application.

## Why Runiq.Core?

`Runiq.Core` is the ASP.NET Core integration package for Runiq.Net.
...
`Runiq.Core` provides the runtime and hosting layer for the broader Runiq.Net platform.
...
Runiq.Net is designed to run inside your application.
...
Use `Runiq.Core` when you want to:

- Add Runiq.Net to an ASP.NET Core application
- Host the embedded dashboard
...
Runiq.Net is modular. `Runiq.Core` is usually used together with other Runiq packages:
