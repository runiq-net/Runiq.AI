...
Code-first workflow orchestration primitives for Runiq.Net.

`Runiq.Workflows` lets you compose agents into repeatable code-first flows. Use it to define workflow steps, model success and failure transitions, validate flow definitions, and expose workflow metadata and execution results through the Runiq.Net runtime and dashboard.
...
`Runiq.Workflows` helps you model these flows in C#.
...
Registered workflows can be surfaced through the Runiq.Net runtime and dashboard.
...
`Runiq.Workflows` supports explicit failure behavior.
...
Use `Runiq.Workflows` when you want to:
...
When used with `Runiq.Core`, workflow definitions and execution metadata can be exposed through the embedded dashboard.
...
Runiq.Net is modular. `Runiq.Workflows` can be used together with other Runiq packages:

| Package | Purpose |
|---|---|
| `Runiq.Agents` | Provides the agent runtime primitives used by workflow steps |
| `Runiq.Core` | Hosts workflow runtime and dashboard endpoints |
