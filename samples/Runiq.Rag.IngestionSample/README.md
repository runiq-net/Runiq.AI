...
```powershell
dotnet run --project samples/Runiq.Rag.IngestionSample/Runiq.Rag.IngestionSample.csproj
```

The sample reads `samples/Runiq.Rag.IngestionSample/sample-document.txt` from disk, creates a `RagDocument`, runs the existing RAG dependency injection setup, chunks the document with the default chunker, and generates embeddings with a deterministic fake provider.
